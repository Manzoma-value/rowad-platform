        ...
        {state === "error" ? (
          ...
          <>
            <header className="fqv-head">
              ...
              <p>Pas përfundimit të fazës së parë të trajnimit, duam të përcaktojmë së bashku mënyrën më të përshtatshme të mbështetjes në terren. Përgjigju pesë pyetjeve të shkurtra.</p>
              <div className="fqv-progress-meta"><span>Pyetja {step + 1} nga {QUESTIONS.length}</span><strong>{Math.round(progress)}%</strong></div>
            ...
            <footer className="fqv-actions">
              ...
              {isLast ? (
                <button type="button" className="primary" disabled={currentAnswer === null || state === "submitting"} onClick={() => void submit()}>
