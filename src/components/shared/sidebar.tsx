...
export default function Sidebar({ title, items }: SidebarProps) {
  return (
    <aside className="sticky top-0 hidden h-screen w-72 shrink-0 overflow-hidden border-l border-[#B8A082]/15 bg-gradient-to-b from-[#5B1526] via-[#4A0E1C] to-[#32101A] p-4 text-[#F7EFE3] shadow-[0_20px_60px_rgba(26,26,26,0.24)] md:block">
      ...
      <nav className="relative space-y-1.5">
        {items.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="group flex min-h-12 items-center gap-3 rounded-2xl border border-transparent px-3 py-2.5 text-sm font-bold text-[#E8DCBC]/75 no-underline hover:border-[#B8A082]/20 hover:bg-white/[0.06] hover:text-[#FFF8E6]"
          >
