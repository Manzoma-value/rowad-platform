...
const S = {
  ar: {
    ...
    contact: "إذا كنت تعتقد أن هذا خطأ، تواصل مع إدارة منصتك.",
    logout: "تسجيل الخروج",
  },
  sq: {
    ...
    <div className="da-shell" dir={dir}>
      <div className="da-card">
        ...
        <div className="da-icon-wrap">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#6B1E2D" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round">
...
.da-shell{
  ...
  background:#EFEAE0;font-family:'Cairo',sans-serif;padding:24px;
}

.da-card{
  background:#FFFBF5;border:1px solid rgba(107,30,45,0.12);border-radius:22px;
...
.da-icon-wrap{
  ...
  background:rgba(107,30,45,0.06);border:1.5px solid rgba(107,30,45,0.14);
  display:flex;align-items:center;justify-content:center;
  margin-bottom:8px;
}

.da-title{
  font-size:24px;font-weight:900;color:#6B1E2D;letter-spacing:-0.3px;
}

.da-msg{
  font-size:15px;color:#4A0E1C;line-height:1.75;font-weight:600;
  max-width:340px;
}

.da-contact{
  font-size:13px;color:#796A62;line-height:1.7;
  max-width:320px;
}

.da-rule{display:flex;align-items:center;gap:10px;width:100%;max-width:200px;margin:8px 0}
.da-rule-line{flex:1;height:1px;background:rgba(107,30,45,0.1)}
.da-rule-diamond{width:5px;height:5px;background:rgba(107,30,45,0.25);transform:rotate(45deg);flex-shrink:0}

.da-btn{
  display:flex;align-items:center;justify-content:center;gap:8px;
  padding:14px 32px;border-radius:12px;
  background:#6B1E2D;color:#FFFBF5;border:none;
  font-family:'Cairo',sans-serif;font-size:14px;font-weight:800;
  cursor:pointer;transition:all 0.18s;width:100%;max-width:260px;
}
.da-btn:hover:not(:disabled){background:#5C1616;transform:translateY(-1px);box-shadow:0 6px 20px rgba(107,30,45,0.2)}
