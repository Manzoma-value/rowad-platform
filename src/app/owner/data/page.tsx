...
export default function OwnerDataRoomPage() {
  ...
  return <div className="dr" dir="rtl">
    ...
    <div className="dr-kpis">{([[Building2, "الجهات", count("schools")], [GraduationCap, "المشرفون", count("teachers")], [BookOpen, "المجموعات", count("classes")], [ShieldCheck, "المدراء", count("admins")]] as [typeof Building2, string, number][]).map(([Icon, label, value]) => <div className="dr-kpi" key={text(label)}><span className="dr-kpi-icon"><Icon size={18} /></span><span><b>{loading ? "—" : Number(value).toLocaleString("en")}</b><small>{text(label)}</small></span></div>)}</div>
    ...
    <div className="dr-panel"><div className="dr-panel-head"><div><h2>{activeLabel}</h2><span>{rows.length.toLocaleString("en")} سجل · آخر تحديث {data ? new Date(data.generatedAt).toLocaleTimeString("ar") : "—"}</span></div><button className="dr-icon-btn" onClick={exportCurrent} title="تصدير"><ArrowDownToLine size={17} /></button></div>{loading ? <div className="dr-empty">جارٍ تجهيز غرفة البيانات...</div> : <div className="dr-table-wrap"><table><thead><tr>{tab === "schools" && <><th>الجهة</th><th>الحالة</th><th>المستخدمون</th><th>المجموعات</th><th /></>}{tab === "teachers" && <><th>المشرف</th><th>الجهة</th><th>الحالة</th><th>الإشارات</th></>}{tab === "classes" && <><th>المجموعة</th><th>الجهة</th><th>المشرف</th><th>التفاعل</th></>}{tab === "admins" && <><th>المدير</th><th>الجهة</th><th>البريد</th><th>الحالة</th></>}{tab === "performance" && <><th>الجهة</th><th>مجموعات</th><th>نماذج قياس</th><th>محاولات</th><th>متوسط الأداء</th></>}</tr></thead><tbody>
      {tab === "schools" && rows.map((x) => <tr key={text(x.id)}><td><strong>{text(x.name)}</strong><small>{text(x.slug)}</small></td><td><em className={x.is_active ? "ok" : "off"}>{x.is_active ? "نشطة" : "متوقفة"}</em></td><td>{text(get(x, "_count.teachers"))} مشرف · {text(get(x, "_count.students"))} مستفيد</td><td>{text(get(x, "_count.classes"))}</td><td><Link href={`/owner/schools/${text(x.id)}`}><ArrowUpRight size={16} /></Link></td></tr>)}
      {tab === "teachers" && rows.map((x) => <tr key={text(x.id)}><td><strong>{text(get(x, "profile.full_name"))}</strong><small>{text(get(x, "profile.email")) || "بدون بريد"}</small></td><td>{text(get(x, "school.name"))}</td><td><em className={get(x, "profile.is_active") ? "ok" : "off"}>{text(x.onboarding_status)}</em></td><td>{text(get(x, "_count.classes"))} مجموعات · {text(get(x, "_count.ratings_received"))} تقييمات</td></tr>)}
      {tab === "classes" && rows.map((x) => <tr key={text(x.id)}><td><strong>{text(x.name)}</strong><small>{text(get(x, "_count.students"))} مستفيد</small></td><td>{text(get(x, "school.name"))}</td><td>{text(get(x, "teacher.profile.full_name")) || "غير مسند"}</td><td>{text(get(x, "_count.assessmentAttempts"))} محاولات · {text(get(x, "_count.quizzes"))} اختبارات</td></tr>)}
