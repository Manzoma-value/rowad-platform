...
export default function OwnerDataRoomPage() {
  ...
  return <div className="dr" dir="rtl">
    ...
    <div className="dr-kpis">{([[Building2, "الجهات", count("schools")], [GraduationCap, "المعلمون", count("teachers")], [BookOpen, "الفصول", count("classes")], [ShieldCheck, "المدراء", count("admins")]] as [typeof Building2, string, number][]).map(([Icon, label, value]) => <div className="dr-kpi" key={text(label)}><span className="dr-kpi-icon"><Icon size={18} /></span><span><b>{loading ? "—" : Number(value).toLocaleString("en")}</b><small>{text(label)}</small></span></div>)}</div>
