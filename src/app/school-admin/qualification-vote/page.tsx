...
export default function QualificationVotePage() {
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      sheet["!cols"] = [{ wch: 25 }, { wch: 30 }, { wch: 24 }, { wch: 22 }, { wch: 22 }, { wch: 20 }, { wch: 20 }, { wch: 24 }];
      ...
      <section className="qv-results">
        ...
        {error ? <div className="qv-message"><p>{error}</p><button type="button" onClick={() => void load()}>إعادة المحاولة</button></div> : (
          <div className="qv-table-wrap" aria-busy={loading}>
            ...
            <table>
              <thead><tr><th>المعلم</th><th>الكوتشينج الفردي</th><th>الاستشارة الجماعية</th><th>التقييم التطويري</th><th>الدعم الميداني</th><th>قائد للمجموعة</th><th>تاريخ الإرسال</th></tr></thead>
              <tbody>
                {!loading && data.votes.length === 0 ? <tr><td colSpan={7} className="qv-empty">لا توجد إجابات مطابقة للفلاتر الحالية.</td></tr> : data.votes.map((vote) => (
                  <tr key={vote.id}>
