...
type Answer = {
  ...
  answer_mode: "SINGLE" | "MULTIPLE" | "NONE";
  answer: string;
                    ...
                    {isOpen && (
                      <div className="vrm-answers">
                        ...
                        {row.answers.map((answer) => {
                          ...
                          const isRecorded = answer.answer_mode === "NONE";
                          return (
                            <section key={answer.id} className={`vrm-answer${isPending ? " pending" : isRecorded ? " recorded" : answer.is_correct ? " correct" : " wrong"}`}>
                              <div className="vrm-answer-icon">
                                {isPending ? <FileText size={17} /> : isRecorded ? <CheckCircle2 size={17} /> : answer.is_correct ? <CheckCircle2 size={17} /> : <XCircle size={17} />}
                              </div>
                              <div className="vrm-answer-content">
                                <div className="vrm-answer-head">
                                  <p>{answer.question_text}</p>
                                  <span>{isPending ? t.awaiting : isRecorded ? t.recorded : answer.is_correct ? t.correct : t.wrong}</span>
                                </div>
                                <blockquote dir="auto">{answer.answer_values?.length ? answer.answer_values.join("، ") : answer.answer}</blockquote>
