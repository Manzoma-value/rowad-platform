// /api/school-admin/teacher-groups/[id]/assessments/[aid]
//   GET    — full payload: members snapshot + every rating row.
//            Aggregation (averages, derived traits) computed client-side
//            so filters/reports can refresh instantly without round trips.
...
export async function GET(
  ...
  const assessment = await prisma.groupAssessment.findFirst({
    where: { id: aid, group_id: id, school_id: auth.school.id },
    select: {
      id: true,
      title: true,
      status: true,
  ...
  // Flatten members for easier client work.
...
export async function DELETE(
  ...
  const existing = await prisma.groupAssessment.findFirst({
    where: { id: aid, group_id: id, school_id: auth.school.id }, select: { id: true },
  });
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });
