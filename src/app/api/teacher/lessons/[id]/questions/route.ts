// api/teacher/lessons/[id]/questions/route.ts
...
// POST /api/teacher/lessons/[id]/questions
export async function POST(
  ...
  const [{ id: lessonId }, body] = await Promise.all([
    context.params,
    req.json().catch(() => ({})),
  ]);
  ...
  if (type === "WRITTEN" && !correct_answer?.trim()) {
    return NextResponse.json({ error: "WRITTEN needs a model answer" }, { status: 400 });
  }
  ...
  const last = await prisma.lessonQuestion.findFirst({
