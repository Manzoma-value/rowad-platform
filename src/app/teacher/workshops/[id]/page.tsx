...
type RequestStatus = "PENDING" | "APPROVED" | "REJECTED" | "WAITLISTED" | null;
...
const text = {
  ar: {
    ...
    requestTitle: "اطلب الانضمام إلى هذه الورشة",
    requestSub: "أرسل طلب انضمام، وسيراجعه مسؤول المنصة. ستُفتح كل الملفات والملاحظات والروابط فور الموافقة.",
    requestBtn: "طلب الانضمام",
    requesting: "جارٍ الإرسال...",
    requestErr: "تعذّر إرسال الطلب. حاول مرة أخرى.",
    pendingTitle: "طلبك قيد المراجعة",
    pendingSub: "أرسلنا طلبك إلى مسؤول المنصة. ستصلك رسالة فور اتخاذ القرار.",
    waitlistedTitle: "أنت على قائمة الانتظار",
    waitlistedSub: "أضافك مسؤول المنصة إلى قائمة الانتظار لهذه الورشة. سنخبرك عند توفر مكان.",
    rejectedTitle: "لم تتم الموافقة على طلبك",
    rejectedSub: "يمكنك إرسال طلب جديد إذا تغيّرت الظروف.",
    requestAgain: "إرسال الطلب مجددًا",
    ...
    navTitle: "تنقّل داخل الورشة",
    navSub: "ابدأ من رحلتك، ثم انتقل مباشرة إلى الجزء الذي تحتاجه.",
    journeyNav: "رحلة الإنجاز",
    videosNav: "الفيديوهات والأسئلة",
  ...
  sq: {
    ...
    requestTitle: "Kërko të bashkohesh me këtë forum",
    requestSub: "Dërgo një kërkesë; administrata e platformës do ta shqyrtojë. Të gjitha materialet hapen menjëherë pas miratimit.",
    requestBtn: "Kërko të bashkohesh",
    requesting: "Duke dërguar...",
    requestErr: "Kërkesa dështoi. Provo përsëri.",
    pendingTitle: "Kërkesa jote është në shqyrtim",
    pendingSub: "Kërkesa u dërgua te administrata e platformës. Do të njoftohesh sapo të merret vendimi.",
    waitlistedTitle: "Je në listën e pritjes",
    waitlistedSub: "Administrata të shtoi në listën e pritjes për këtë forum. Do të njoftohesh kur të lirohet vend.",
    rejectedTitle: "Kërkesa jote nuk u miratua",
    rejectedSub: "Mund të dërgosh një kërkesë të re nëse rrethanat kanë ndryshuar.",
    requestAgain: "Dërgo kërkesën përsëri",
    ...
    navTitle: "Lëviz brenda forumit",
    navSub: "Fillo me rrugëtimin dhe shko drejtpërdrejt te pjesa që të duhet.",
    journeyNav: "Rrugëtimi i përfundimit",
    videosNav: "Videot dhe pyetjet",
...
export default function TeacherWorkshopDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function requestToJoin() {
    ...
    setRequestError("");
    try {
      const response = await fetch(`/api/teacher/workshops/${id}/request`, { method: "POST" });
      if (!response.ok) throw new Error("request_failed");
      await load(true);
    } catch {
      setRequestError(T.requestErr);
    } finally {
      setRequesting(false);
    }
  }
      ...
      <section id="workshop-materials" className="tw-section">
        ...
        {!data.has_access ? (
          <div className="tw-locked">
            {data.request_status === "PENDING" ? (
            ...
            ) : data.request_status === "WAITLISTED" ? (
              <>
                <Clock3 size={28} />
                <strong>{T.waitlistedTitle}</strong>
                <p>{T.waitlistedSub}</p>
              </>
              ...
              <>
                ...
                <p>{T.rejectedSub}</p>
                <button className="tw-request-btn" onClick={() => void requestToJoin()} disabled={requesting}>
                  <Send size={15} />{requesting ? T.requesting : T.requestAgain}
                </button>
                {requestError && <p className="tw-request-err">{requestError}</p>}
              </>
            ) : (
              <>
                <LockKeyhole size={28} />
                ...
                <p>{T.requestSub}</p>
                <button className="tw-request-btn" onClick={() => void requestToJoin()} disabled={requesting}>
                  <Send size={15} />{requesting ? T.requesting : T.requestBtn}
                </button>
                {requestError && <p className="tw-request-err">{requestError}</p>}
                <p className="tw-locked-alt">{T.lockedSub}</p>
              </>
            )}
          </div>
            ...
            {workshop.messages.length === 0 ? (
              ...
              return (
                <article className={`tw-message${isAdmin ? " admin" : ""}`} key={message.id}>
                  <ProfileAvatar author={message.author} lang={locale} size={34} />
