  ...
  return (
    <div className="td-page" dir={dir}>
      ...
      <main className="td-shell">
        ...
        <section className="td-command-row">
          <div className="td-command-panel td-my-groups">
            <div className="td-command-head">
              <div>
                <span className="td-section-label">{tr.openGroups}</span>
                <h2>{tr.groups}</h2>
              </div>
            ...
            {(data.dashboard?.groups.length ?? 0) === 0 ? (
              ...
              <div className="td-groups-table">
                {data.dashboard?.groups.slice(0, 6).map((group) => (
                  <Link key={group.id} href={`/teacher/groups/${group.id}`} className="td-group-row">
                    <span className="td-group-name">{group.name}</span>
                    <span>{group.member_count} {tr.members}</span>
                    <span>{group.announcement_count} {tr.announcements}</span>
                    <span>{group.assessment_count} {tr.assessments}</span>
                  </Link>
                ))}
              </div>
            )}
          ...
          <Link href="/teacher/hub" className="td-command-panel td-community-card">
            <div className="td-command-head">
              <div>
                <span className="td-section-label">{tr.communityPulse}</span>
                <h2>{tr.openCommunity}</h2>
              </div>
              <span className="td-community-count">{data.dashboard?.community_posts.length ?? 0}</span>
            </div>
            {(data.dashboard?.community_posts.length ?? 0) === 0 ? (
              <p className="td-muted">{tr.noCommunity}</p>
            ) : (
              <div className="td-community-preview">
                {data.dashboard?.community_posts.slice(0, 2).map((post) => (
                  <article key={post.id}>
...
.td-page{min-height:100%;position:relative;overflow:hidden;background:linear-gradient(180deg,#F7F2E8 0%,#EFE6D8 100%);font-family:Cairo,Tajawal,sans-serif;color:#32101A}
...
.td-hero{display:block;background:linear-gradient(135deg,#09111B,#172231 68%,#31250F);border:1px solid rgba(184,160,130,.30);border-radius:18px;padding:26px;box-shadow:0 18px 40px rgba(20,14,6,.18);overflow:hidden}
.td-kicker,.td-section-label{font-size:11px;font-weight:900;letter-spacing:.12em;text-transform:uppercase;color:#B8A082}.td-hero h1{margin:8px 0 8px;font-size:clamp(26px,4vw,48px);line-height:1.05;color:#F7F3EB;letter-spacing:-.04em}.td-hero p{max-width:680px;color:rgba(247,237,216,.74);font-size:15px;line-height:1.8}
.td-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:22px}.td-action,.td-mini-link,.td-quick a{display:inline-flex;align-items:center;justify-content:center;text-decoration:none;border-radius:10px;font-weight:900;transition:.18s ease}.td-action{padding:10px 15px;color:#F7F3EB;border:1px solid rgba(247,237,216,.18);background:rgba(255,255,255,.06)}.td-action.primary{color:#32101A;background:linear-gradient(135deg,#F0CE70,#B8A082);border-color:transparent}.td-action:hover,.td-mini-link:hover,.td-quick a:hover{transform:translateY(-1px)}
.td-orbit{position:relative;min-height:210px;display:grid;place-items:center}.td-orbit-ring{position:absolute;width:210px;height:210px;border-radius:50%;border:1px solid rgba(184,160,130,.20);box-shadow:inset 0 0 0 18px rgba(255,255,255,.03)}.td-orbit-core{width:104px;height:104px;border-radius:32px;background:linear-gradient(145deg,#F7F3EB,#B8A082);display:grid;place-items:center;font-size:32px;font-weight:900;color:#32101A;box-shadow:0 18px 36px rgba(0,0,0,.26)}.td-orbit-chip{position:absolute;padding:7px 11px;border-radius:999px;background:rgba(255,255,255,.10);border:1px solid rgba(247,237,216,.18);color:#F7F3EB;font-size:12px;font-weight:900;backdrop-filter:blur(10px)}.td-orbit-chip.c1{top:18px;inset-inline-start:18px}.td-orbit-chip.c2{bottom:22px;inset-inline-end:14px}.td-orbit-chip.c3{top:70px;inset-inline-end:0}
.td-stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.td-stat-card{position:relative;overflow:hidden;border-radius:14px;background:#FFFBF5;border:1px solid rgba(184,155,94,.20);padding:16px;box-shadow:0 10px 24px rgba(42,26,10,.06)}.td-stat-card::after{content:"";position:absolute;inset-inline-end:0;top:0;width:4px;height:100%;background:#B8A082}.td-stat-card span{font-size:12px;font-weight:900;color:#8E7243}.td-stat-card strong{display:block;margin-top:8px;font-size:32px;line-height:1;color:#32101A}.td-stat-card small{display:block;margin-top:8px;color:#796A62;font-weight:700}
.td-command-row{display:grid;grid-template-columns:minmax(0,1.45fr) minmax(320px,.75fr);gap:14px;align-items:stretch}
.td-command-panel{display:block;text-decoration:none;color:#32101A;background:#FFFBF5;border:1px solid rgba(184,155,94,.20);border-radius:16px;padding:16px;box-shadow:0 12px 28px rgba(42,26,10,.07)}
.td-command-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba(184,155,94,.14)}
.td-command-head h2{margin:2px 0 0;font-size:18px;font-weight:900;color:#32101A}
.td-groups-table{display:grid;gap:8px}
.td-group-row{display:grid;grid-template-columns:minmax(170px,1fr) repeat(3,auto);gap:14px;align-items:center;text-decoration:none;color:#32101A;border:1px solid rgba(184,155,94,.16);border-radius:12px;background:#fff;padding:11px 12px;transition:.16s ease}
.td-group-row:hover{border-color:rgba(184,160,130,.55);background:#FBF6EA;transform:translateY(-1px)}
.td-group-row span:not(.td-group-name){font-size:12px;font-weight:900;color:#796A62;white-space:nowrap}
.td-group-name{font-size:14px;font-weight:900;color:#172231;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.td-community-card{background:linear-gradient(150deg,#FFFBF5,#F7EEDC)}
.td-community-count{width:34px;height:34px;border-radius:11px;background:#09111B;color:#D8B760;display:grid;place-items:center;font-weight:900}
.td-community-preview{display:grid;gap:9px}
.td-community-preview article{border:1px solid rgba(184,155,94,.16);border-radius:12px;background:#fff;padding:12px}
.td-community-preview strong{display:block;font-size:13px;color:#172231}.td-community-preview p{margin:5px 0;color:#4A0E1C;line-height:1.6;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.td-community-preview small{color:#796A62;font-weight:900}
.td-grid{display:grid;grid-template-columns:320px minmax(0,1fr) 360px;gap:14px;align-items:start}.td-panel{background:#FFFBF5;border:1px solid rgba(184,155,94,.18);border-radius:16px;padding:16px;box-shadow:0 12px 28px rgba(42,26,10,.07);backdrop-filter:blur(18px)}.td-panel-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:14px;padding-bottom:10px;border-bottom:1px solid rgba(184,155,94,.14)}.td-panel-head h2{margin:2px 0 0;font-size:18px}.td-panel-head.compact h2{font-size:16px}.td-mini-link{padding:7px 11px;background:#09111B;color:#B8A082;font-size:12px;white-space:nowrap}.td-soft-pill{border-radius:999px;background:rgba(184,160,130,.14);color:#775922;padding:6px 10px;font-size:12px;font-weight:900;white-space:nowrap}
.td-class-list,.td-group-list,.td-feed-list,.td-ann-list,.td-side-stack{display:flex;flex-direction:column;gap:10px}.td-class-row{width:100%;display:flex;align-items:center;gap:10px;border:1px solid rgba(184,155,94,.15);border-radius:18px;background:#fff;padding:11px;cursor:pointer;font-family:inherit;text-align:start;transition:.18s ease}.td-class-row:hover,.td-class-row.active{border-color:rgba(184,160,130,.45);background:rgba(184,160,130,.10);transform:translateY(-1px)}.td-class-mark,.td-avatar{width:38px;height:38px;border-radius:14px;background:#32101A;color:#B8A082;display:grid;place-items:center;font-weight:900;flex-shrink:0}.td-class-row strong{display:block;font-size:13px}.td-class-row small{color:#796A62;font-weight:700}.td-class-arrow{margin-inline-start:auto;color:#B8A082;font-size:24px}
...
.td-compose{display:flex;flex-direction:column;gap:12px;border-radius:22px;padding:14px;background:linear-gradient(145deg,#fff,rgba(246,240,230,.70));border:1px solid rgba(184,155,94,.16)}.td-compose-top,.td-ann-meta{display:flex;align-items:center;gap:9px}.td-avatar.sm{width:28px;height:28px;border-radius:10px;font-size:10px}.td-compose textarea{width:100%;border:1.5px solid rgba(184,155,94,.18);border-radius:17px;background:#FFFBF5;padding:12px 14px;resize:none;outline:none;font:inherit;font-size:16px;line-height:1.7;color:#32101A}.td-compose textarea:focus{border-color:rgba(184,160,130,.55);box-shadow:0 0 0 4px rgba(184,160,130,.12)}.td-compose-foot{display:flex;justify-content:space-between;align-items:center;color:#8B7B62;font-size:12px;font-weight:800}.td-compose-foot button{border:0;border-radius:999px;background:#32101A;color:#B8A082;padding:10px 16px;font:inherit;font-weight:900;cursor:pointer}.td-compose-foot button:disabled{opacity:.45;cursor:not-allowed}
.td-ann{display:flex;gap:12px;align-items:flex-start;border:1px solid rgba(184,155,94,.15);border-radius:20px;background:#fff;padding:13px}.td-ann.muting{opacity:.45}.td-ann-main{flex:1}.td-ann-meta small{color:#89765A;font-weight:700}.td-ann p{margin:9px 0 0;line-height:1.75;color:#4A0E1C}.td-ann button{border:1px solid rgba(184,155,94,.22);background:rgba(246,240,230,.70);border-radius:999px;padding:7px 11px;font:inherit;font-size:12px;font-weight:900;cursor:pointer;color:#775922}.td-empty-mini,.td-muted{border-radius:18px;background:rgba(246,240,230,.68);padding:16px;color:#796A62;font-weight:800;line-height:1.7}
.td-group-card,.td-feed-item{display:block;text-decoration:none;border-radius:18px;background:#fff;border:1px solid rgba(184,155,94,.16);padding:12px;transition:.18s ease;color:#32101A}.td-group-card:hover,.td-feed-item:hover{transform:translateY(-1px);border-color:rgba(184,160,130,.42)}.td-group-card strong,.td-feed-item span{display:block;font-weight:900}.td-group-card span,.td-feed-item small{color:#796A62;font-size:12px;font-weight:800}.td-feed-item p{margin:5px 0;color:#4A0E1C;line-height:1.55;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.td-quick{border-radius:26px;padding:18px;background:linear-gradient(135deg,#32101A,#3D2D14);box-shadow:0 18px 44px rgba(20,14,6,.20)}.td-quick h2{margin:0 0 12px;color:#F7F3EB;font-size:18px}.td-quick a{width:100%;margin-top:8px;padding:10px 12px;background:rgba(255,255,255,.08);border:1px solid rgba(247,237,216,.14);color:#F7F3EB}
.td-strip{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.td-strip-card{border-radius:20px;background:rgba(255,253,248,.70);border:1px solid rgba(184,155,94,.16);padding:14px}.td-strip-card strong{display:block}.td-strip-card span{font-size:12px;color:#796A62;font-weight:800}.td-empty{background:rgba(255,253,248,.86);border:1px solid rgba(184,155,94,.18);border-radius:26px;padding:46px;text-align:center}.td-empty-icon{margin:0 auto 12px;width:50px;height:50px;border-radius:18px;display:grid;place-items:center;background:#32101A;color:#B8A082;font-weight:900}.td-empty h2{margin:0 0 6px}.td-empty p{color:#796A62;font-weight:800}
...
@media(max-width:1180px){.td-grid{grid-template-columns:310px minmax(0,1fr)}.td-side-stack{grid-column:1/-1;display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.td-quick{grid-column:1/-1}.td-hero,.td-command-row{grid-template-columns:1fr}}
@media(max-width:760px){.td-shell{padding:16px 12px 34px}.td-hero{padding:22px;border-radius:18px}.td-orbit{display:none}.td-stats,.td-grid,.td-side-stack,.td-strip,.td-skel-grid{grid-template-columns:1fr}.td-panel{border-radius:16px;padding:14px}.td-actions{gap:8px}.td-action{flex:1;min-width:140px}.td-ann{flex-direction:column}.td-ann button{align-self:flex-start}.td-group-row{grid-template-columns:1fr 1fr;gap:7px}.td-group-name{grid-column:1/-1}}
`;
